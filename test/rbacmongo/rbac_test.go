...
func init() {
	...
	opts = opts.ApplyURI(uri).SetAuth(options.Credential{
		Username: "mongoadmin",
		Password: "secret",
	})
}
func TestEmptyAuth(t *testing.T) {
	err := auth.NewMongo(opts, "rbactest")
	if err != nil {
		t.Fatal(err)
	}
	auth.Clear()
...
func TestRegExPermission(t *testing.T) {
	...
	granted := auth.IsGranted("rbactest", rbac2.RBACPermission{
		Name: "get:test",
	}, nil)
	if !granted {
		t.Fatal("problem with permission grant")
